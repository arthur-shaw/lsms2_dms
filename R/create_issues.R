...
create_outlier_issues <- function(
  ...
  non_food_7d_specs <- tibble::tribble(
    ~ var, ~ by,
    "hh09a_q03", "non_food_7_days__id", 
  ) |>
	dplyr::rowwise() |>
  # TODO: update message reference
  dplyr::mutate(desc = get_msg("outliers", "non_food_7d", var)) |>
  dplyr::ungroup()
  ...
  non_food_6m_specs <- tibble::tribble(
    ~ var, ~ by,
    "hh09c_q03", "non_food_6_months__id",
  ) |>
	dplyr::rowwise() |>
  dplyr::mutate(desc = get_msg("outliers", "non_food_6m", var)) |>
  dplyr::ungroup()
  ...
  non_food_12m_specs <- tibble::tribble(
    ~ var, ~ by,
    "hh09d_q03", "non_food_12_months__id", 
  ) |>
	dplyr::rowwise() |>
  dplyr::mutate(desc = get_msg("outliers", "non_food_12m", var)) |>
  dplyr::ungroup()
